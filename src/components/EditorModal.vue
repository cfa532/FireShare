...
async function onSubmit() {
  if (
    !inpCaption.value ||
    inpCaption.value!.trim() === '' ||
    (filesUpload.value.length === 0 && textValue.value.trim() === '')
  ) {
  ...
  try {
    if (filesUpload.value.length) {
      ...
      let fvPairs: FVPair[] = (await uploadFile(filesUpload.value))
        ...
        .map((v: any) => {
          return { field: v.value.mid, value: v.value }
        }) // mid as field, fileInfo as value
      ...
      if (fvPairs.length === 1 && textValue.value.trim() === '') {
        ...
        let ret = await api.client.Zadd(
          mmsidCur,
          props.column,
          new ScorePair(Date.now(), fvPairs[0].value['mid'])
        )
